...
from hydrangea.protocols.protocol_state_machine import message_requires_reply
from hydrangea.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
from hydrangea.protocols.shared_protocol import protocol_version
from hydrangea.server.introducer_peers import IntroducerPeers
from hydrangea.server.outbound_message import Message, NodeType
from hydrangea.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from hydrangea.util.errors import Err, ProtocolError
from hydrangea.util.ints import uint16
from hydrangea.util.network import is_in_network, is_localhost, select_port
from hydrangea.util.ssl_check import verify_ssl_certs_and_keys
...
class HydrangeaServer:
    def __init__(
        ...
        hydrangea_ca_crt_key: Tuple[Path, Path],
        name: str = None,
    ):
        # Keeps track of all connections to and from this node.
        self.all_connections: Dict[bytes32, WSHydrangeaConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSHydrangeaConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.DATA_LAYER: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
        ...
        self.hydrangea_ca_crt_path, self.hydrangea_ca_key_path = hydrangea_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSHydrangeaConnection] = []
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.hydrangea_ca_crt_path, self.hydrangea_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSHydrangeaConnection] = None
        try:
            connection = WSHydrangeaConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSHydrangeaConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.hydrangea_ca_crt_path, self.hydrangea_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSHydrangeaConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSHydrangeaConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSHydrangeaConnection, task_id):
    ...
    async def send_to_others(
        ...
        node_type: NodeType,
        origin_peer: WSHydrangeaConnection,
    ):
    ...
    def get_outgoing_connections(self) -> List[WSHydrangeaConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSHydrangeaConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSHydrangeaConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSHydrangeaConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSHydrangeaConnection, trusted_peers: Dict) -> bool:
