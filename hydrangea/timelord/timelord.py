...
class Timelord:
        ...
        for potential_sp_iters, signage_point_index in self.signage_point_iters:
            ...
            if len(proofs_with_iter) == 2:
                ...
                if "xhg_target_address" not in self.config:
                    log.warning("xhg_target_address missing in the config, rewards will be lost")
                    timelord_reward_puzzle_hash: bytes32 = self.constants.GENESIS_PRE_FARM_TIMELORD_PUZZLE_HASH
                else:
    ...
    async def _handle_failures(self):
        ...
        if time.time() - self.vdf_failure_time < self.constants.SUB_SLOT_TIME_TARGET * 3:
            ...
            active_time_threshold = self.config.get("active_time_threshold", 60)
