...
from hydrangea.plot_sync.delta import Delta
from hydrangea.plot_sync.receiver import Receiver
from hydrangea.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from hydrangea.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from hydrangea.util.errors import KeychainProxyConnectionFailure
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        ...
        if "xhg_target_address" not in self.config:
            self.config = config["farmer"]
        if "xhg_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xhg_target_address" not in self.config or "xhg_target_address" not in self.pool_config:
            log.debug("xhg_target_address missing in the config")
            return False
