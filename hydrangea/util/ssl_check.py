...
import stat
import sys
from hydrangea.util.config import load_config, traverse_dict
from hydrangea.util.permissions import octal_mode_string, verify_file_permissions
...
CERT_CONFIG_KEY_PATHS = [
    ...
    "harvester:hydrangea_ssl_ca:crt",
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `hydrangea init --fix-ssl-permissions` to fix issues.")
