from hydrangea.protocols import full_node_protocol, introducer_protocol, wallet_protocol
...
from hydrangea.util.api_decorators import api_request, peer_required, execute_task
...
class WalletNodeAPI:
    ...
    @peer_required
    @api_request
    async def respond_removals(self, response: wallet_protocol.RespondRemovals, peer: WSHydrangeaConnection):
        pass

    async def reject_removals_request(self, response: wallet_protocol.RejectRemovalsRequest, peer: WSHydrangeaConnection):
        """
    ...
    @peer_required
    @api_request
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSHydrangeaConnection):
        """
    ...
    @peer_required
    @api_request
    async def respond_additions(self, response: wallet_protocol.RespondAdditions, peer: WSHydrangeaConnection):
        pass
    ...
    @peer_required
    @api_request
    async def transaction_ack(self, ack: wallet_protocol.TransactionAck, peer: WSHydrangeaConnection):
        """
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSHydrangeaConnection
    ):
    ...
    @peer_required
    @api_request
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: WSHydrangeaConnection):
    ...
    @peer_required
    @api_request
    async def coin_state_update(self, request: wallet_protocol.CoinStateUpdate, peer: WSHydrangeaConnection):
