...
# the new passphrase.
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ hydrangea passphrase set # all the cool kids are doing it!"

MASTER_PASSPHRASE_SERVICE_NAME = "Hydrangea Passphrase"
MASTER_PASSPHRASE_USER_NAME = "Hydrangea Passphrase"
...
def check_legacy_keyring_keys_present(keyring: LegacyKeyring) -> bool:
    ...
    from hydrangea.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def _get_initial_cached_passphrase(self) -> str:
        ...
        """
        from hydrangea.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'hydrangea passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase?")
            if response:
                from hydrangea.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'hydrangea passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from hydrangea.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    async def migrate_legacy_keyring_interactive(self) -> bool:
        ...
        """
        from hydrangea.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
