...
from hydrangea.daemon.client import acquire_connection_to_daemon
from hydrangea.util.config import load_config
from hydrangea.util.errors import KeychainMaxUnlockAttempts
from hydrangea.util.keychain import Keychain, supports_os_passphrase_storage
from hydrangea.util.keyring_wrapper import KeyringWrapper, DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Hydrangea processes will be able to access your keys without prompting for your passphrase."
            )
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'hydrangea passphrase set' or 'hydrangea passphrase remove' to update or remove your passphrase")
        sys.exit(1)
