...
from hydrangea.cmds.wallet_funcs import print_balance, wallet_coin_unit
from hydrangea.pools.pool_config import load_pool_config, PoolWalletConfig, update_pool_config
from hydrangea.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from hydrangea.protocols.pool_protocol import POOL_PROTOCOL_VERSION
...
from hydrangea.util.bech32m import encode_puzzle_hash, decode_puzzle_hash
...
from hydrangea.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg, get_any_service_client
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["hydrangea"]))
    ...
    if user_input.lower() == "y" or user_input.lower() == "yes":
        try:
            ...
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  hydrangea start -r farmer")
        return
    print("Aborting.")
