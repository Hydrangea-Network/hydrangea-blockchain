...
There is also a provided NodeClient object which implements many of the methods from hydrangea.rpc.full_node_rpc_client
...
class SpendSim:
    ...
    async def farm_block(self, puzzle_hash: bytes32 = bytes32(b"0" * 32)) -> Tuple[List[Coin], List[Coin]]:
        ...
        staking_coin: Coin = create_staking_coin(
            next_block_height,
            puzzle_hash,
            calculate_staking_reward(next_block_height),
            self.defaults.GENESIS_CHALLENGE,
        )
        community_coin: Coin = create_community_coin(
            next_block_height,
            puzzle_hash,
            calculate_community_reward(next_block_height),
            self.defaults.GENESIS_CHALLENGE,
        )
        ...
        await self.mempool_manager.coin_store._add_coin_records(
            ...
            self.new_coin_record(staking_coin, True),
            self.new_coin_record(community_coin, True),
