...
## 1.6.0 Hydrangea blockchain 2022-9-20
...
## 1.5.1 Hydrangea blockchain 2022-8-23
...
- Add `hydrangea db backup --backup-file <backup_file_destination>` (Thanks @neurosis69!)
...
- Remove hydrangea.util.path.mkdir()
...
- Removing `logging.basicConfig()` from `HydrangeaServer.__init__()`
...
- Log `hydrangea_full_version_str` in daemon and all services
...
- Substantially reduced overall size of Hydrangea packages
...
- Make sure hydrangea commands output help when no args are given (#11013) (Thanks @noneus!)
...
## 1.5.0 Hydrangea blockchain 2022-7-26
...
- Update `hydrangea wallet take_offer` to show NFT royalties that will be paid out when an offer is taken
- Added a parameter to indicate how many additional puzzle hashes `create_more_puzzle_hashes` should create

### Fixed

- Fixed [CVE-2022-36447] where in tokens previously minted on the Hydrangea blockchain using the `CAT1` standard can be inflated in arbitrary amounts by any holder of the token. Total amount of the token can be increased as high as the malicious actor pleases. This is true for every `CAT1` on the Hydrangea blockchain, regardless of issuance rules. This attack is auditable on-chain, so maliciously altered coins can potentially be "marked" by off-chain observers as malicious.
...
## 1.4.0 Hydrangea blockchain 2022-6-29
...
- New coin selection algorithm based on bitcoin knapsack. Previously hydrangea selected the largest coin
...
- Updated warning about `HYDRANGEA_ROOT` being set when running init
...
## 1.3.5 Hydrangea blockchain 2022-5-11
...
## 1.3.4 Hydrangea blockchain 2022-4-19
...
- Implemented `hydrangea rpc` command
...
- Added `-n`/`--new-address` option to `hydrangea wallet get_address`
...
- Fixed typo in command line argument parsing for `hydrangea db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `hydrangea configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `hydrangea db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `hydrangea show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `hydrangea wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `hydrangea wallet show`
...
- You cannot install and run hydrangea blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
## 1.3.3 Hydrangea blockchain 2022-4-02
...
- Fixed exception when `hydrangea keys migrate` is run without needing migration.
...
- Added checks to ensure wallet address prefixes are either `xhg` or `txhg`.
...
## 1.3.0 Hydrangea blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of Hydrangea (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores Hydrangea will not use when launching process pools. Using 0 will allow Hydrangea to use all cores for process pools. Set the default to 0 to allow Hydrangea to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, hydrangea keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using hydrangea keys derive search.
...
- CLI option hydrangea show will display the currently selected network (mainnet or testnet).
- CLI option hydrangea plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Fixed issues with displaying IPv6 address in CLI with hydrangea show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the hydrangea-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI hydrangea show -c when No Info should be displayed.
...
- When you close the Hydrangea app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the Hydrangea and relaunch the Hydrangea app.
...
## 1.2.11 Hydrangea blockchain 2021-11-4
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the Hydrangea Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `hydrangea init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `hydrangea plotnft join`) now accept a fee parameter, but it is not yet operable.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other hydrangea services.
...
- Updated the old comment on hydrangea/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Hydrangea DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 Hydrangea blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Hydrangea blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Hydrangea's broader set of DID capabilities.
...
- Several clvm_rs updates to support our upcoming Hydrangea Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install hydrangea-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the hydrangea install and then converts it into proper npm format and puts that into package.json.
...
## 1.2.3 Hydrangea blockchain 2021-07-26
...
- Thanks @mishan for your change to 'hydrangea plotnft show' to display Percent Successful Points.
...
## 1.2.2 Hydrangea blockchain 2021-07-13
...
## 1.2.0 Hydrangea blockchain 2021-07-07
...
- All Hydrangea repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `hydrangea plotnft` to manage pools.
...
- Correctly display private key in `hydrangea keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `hydrangea keys show`.
...
- Thank you to @HydrangeaMineJP for various improvements.
...
- `hydrangea farm summary` better handles wallet errors.
...
### 1.1.7 Hydrangea Blockchain 2021-06-05
...
## 1.1.6 Hydrangea Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /hydrangea/util/profiler.py for instructions).
...
## 1.1.5 Hydrangea Blockchain 2021-05-09
...
## 1.1.4 Hydrangea Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `hydrangea keys show`. Use `hydrangea keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Hydrangea Blockchain 2021-05-01
...
## 1.1.2 Hydrangea Blockchain 2021-04-24
...
## 1.1.1 Hydrangea Blockchain 2021-04-21
...
## 1.1.0 Hydrangea Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Hydrangea Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
...
## 1.0.5 Hydrangea Blockchain 2021-04-14
...
## 1.0.4 Hydrangea Blockchain 2021-04-12
...
- The `src` directory in hydrangea-blockchain has been changed to `hydrangea` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `hydrangea/`
...
## 1.0.3 Hydrangea Blockchain 2021-03-30
...
- An incorrect merge brought in unreleased features and broke `hydrangea keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 Hydrangea Blockchain 2021-03-30
...
- @jespino changed `hydrangea keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `hydrangea keys show`.
...
## 1.0.1 Hydrangea Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `hydrangea keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/hydrangea-blockchain/).
...
- Users can now pip install e.g. hydrangea-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of Hydrangea Blockchain 2021-03-17

### Added

- This is the first production release of the Hydrangea Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable XHG. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/hydrangea-blockchain).
...
- Mainnet uses port 8255 and other constants and service names were changed for mainnet.
...
- Migration with HYDRANGEA_ROOT set does not crash hydrangea init
...
- `hydrangea farm summary` will now display TXHG or XHG as appropriate.
...
- `hydrangea farm summary` displayed some incorrect amounts.
...
- There was a typo in `hydrangea plots`. Thanks @adamfiddler.
...
- Remove `hydrangea plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
...
- The Farm page will now no longer get stuck at 50 TXHG farmed.
- `hydrangea farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `hydrangea wallet` had various bugs.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `xhg_target_address` sections of config.yaml.
...
- `hydrangea wallet` now takes get_address to get a new wallet receive address from the CLI.
- `hydrangea plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of hydrangea-blockchain, etc., instead of in blspy.
...
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in hydrangea-blockchain.
...
- Various typos and style clean ups were made to the Click CLI implementation. `hydrangea -upnp f` was added to disable uPnP.
- `hydrangea plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major chialisp changes. TXHG from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/hydrangea-blockchain).
- You can obtain a new wallet receive address on the command line with `hydrangea wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- `hydrangea init` only migrates release candidate directories. The versioned sub directories under `~/hydrangea` will be going away before mainnet.
...
- Installers will now be of the pattern HydrangeaSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'hydrangea wallet get_transactions' in the command line to see your transactions.
- 'hydrangea wallet show' now shows your wallet's height.
...
- 'hydrangea wallet send' now takes in TXHG or XHG as units instead of mojos.
...
- The Beta 27 chain is a hard fork. All TXHG from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- You can now send and receive transactions with the command line. Try `hydrangea wallet -h` to learn more. Also, `hydrangea wallet` now requires a third argument of `show`, therefor you will use `hydrangea wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [hydrangea blockchain gui](https://crowdin.com/project/hydrangea-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `hydrangea init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `hydrangea init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- The cli wallet, `hydrangea wallet`, was incorrectly displaying TXHG balances as if they were Coloured Coins.
...
- `hydrangea plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.
...
- `hydrangea plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- We have removed the '-r' flag from `hydrangea` as it was being overridden in most cases by the `-r` for restart flag to `hydrangea start`. Use `hydrangea --root-path` instead.
- `hydrangea -h` now recommends `hydrangea netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
...
- `hydrangea plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `hydrangea plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `hydrangea show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `hydrangea show` handles sub block 0 better.
...
- `hydrangea plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `hydrangea plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- You can now specify which private key to use for `hydrangea plots create`. After obtaining the fingerprint from `hydrangea keys show`, try `hydrangea plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TXHG you've won so far. Harvester and Full Node connections have moved to Advanced Options.
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Hydrangea application set to launch on start.
- Testnet is now running at the primary port of 58255. Update your routers appropriately. This opens 8255 for mainnet.
- All networking code has been refactored and mostly moved to websockets.
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.hydrangea/VERSION/config/`
...
- Tests now place a cache of blocks and plots in the ~/.hydrangea/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run hydrangea-blockchain.
...
- `hydrangea show -s` now is less brain dead when a node is initially starting to sync.
...
- The rpc interfaces of all hydrangea services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test hydrangea since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve hydrangea-blockhydrangean's Pypi entry. Thanks to @altendky for this pull request.
...
- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test hydrangea are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.hydrangea` directory.
...
- `hydrangea init` now automatically discovers previous releases in each new release.

### Fixed

- `hydrangea show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `hydrangea show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Hydrangea Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Hydrangea UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (hydrangea run farmer-only, wallet-only, timelord-only)
...
- `hydrangea show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `hydrangea show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Hydrangea simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Hydrangea implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test hydrangea between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test hydrangea from the earlier two releases however.
...
- `hydrangea show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Hydrangea)
...
- See wallet balances in command line: `hydrangea show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `hydrangea keys sign` and `hydrangea keys verify` to allow farmers to certify their ownership of keys.
...
- `hydrangea start wallet-server` changed to `hydrangea start wallet`, for consistency.
...
- Removed legacy scripts such as hydrangea-stop-server, hydrangea-restart-harvester, etc.
...
- The hydrangea executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./hydrangea -h` from
`~\AppData\Local\Hydrangea-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Hydrangea.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.

### Changed

- Minor changes have been made across the repositories to better support
compiling on OpenBSD. HT @n1000.
- Changed XHG units to TXHG units for testnet.
...
"hydrangea plots create" and "hydrangea plots check".
- Add plot directories to config.yaml from the cli with "hydrangea plots add".
- Use real plot sizes in UI instead of a formula/
- HD keys now use EIP 2333 format instead of BIP32, for compatibility with
other chains.
- Keys are now derived with the EIP 2334 (m/12381/8255/a/b).
...
- The Windows installer is now signed by a Hydrangea Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.hydrangea/test-plots/`
- We have completely refactored all networking code towards making each Hydrangea service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make hydrangea start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Hydrangea Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting hydrangea now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'hydrangea keys' command line.
...
- We added a `hydrangea keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the hydrangea-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `hydrangea-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `hydrangea-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `hydrangea-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `hydrangea-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `hydrangea init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `hydrangea start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `hydrangea-start-harvester`.
...
- The user interface now refers to hydrangea coins with their correct currency code of XHG.
...
- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `hydrangea init` then sync on first start will happen automatically. "\$HYDRANGEA_ROOT" users will need to delete `$HYDRANGEA_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- The script `hydrangea start wallet-gui` has been chaned to `hydrangea start wallet` which launches but the GUI and server on MacOS and Linux. `hydrangea start wallet-server` remains for WSL 2 and Windows native.
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `hydrangea stop all` command sometimes fails, use `hydrangea-stop-all` instead. In windows, use the task manager to stop the servers.
...
- We added full block lookup to `hydrangea show`.

### Changed

- `hydrangea-restart-harvester` has been renamed from `hydrangea-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.hydrangea/beta-1.0b3/config/plots.yaml` after migrating
...
- New AJAX based full node UI. To access go to [http://127.0.0.1:8355/index.html](http://127.0.0.1:8355/index.html) with any modern web browser on the same machine as the full node.
...
- Hydrangea fullnode, farmer and harvester now default to logging to hydrangea.log in the hydrangea-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Hydrangea blockchain.
- Implemented an RPC interface with JSON serialization for streamables - currently on port 8355.
- Added details on how to contribute in CONTRIBUTING.md. Thanks @RichardLitt.
- Added color logging
- Now hydrangea_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to hydrangea_full node, hydrangea_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new hydrangea services were started.
...
- Introducer now makes sure it only sends peer addresses to peers of peers that it can reach on port 8255 or their UPnP port.
