...
from hydrangea.cmds.wallet_funcs import print_offer_summary
...
@pytest.mark.asyncio
async def test_print_offer_summary_xhg(capsys: Any) -> None:
    summary_dict = {"xhg": 1_000_000_000_000}

    await print_offer_summary(cat_name_resolver, summary_dict)

    captured = capsys.readouterr()

    assert "XHG (Wallet ID: 1): 1.0 (1000000000000 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xhg_and_cats(capsys: Any) -> None:
    summary_dict = {
        "xhg": 2_500_000_000_000,
    ...
    assert "XHG (Wallet ID: 1): 2.5 (2500000000000 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xhg_and_cats_with_zero_values(capsys: Any) -> None:
    summary_dict = {
        "xhg": 0,
    ...
    assert "XHG (Wallet ID: 1): 0.0 (0 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xhg_with_one_mojo(capsys: Any) -> None:
    summary_dict = {"xhg": 1}
    ...
    assert "XHG (Wallet ID: 1): 1e-12 (1 mojo)" in captured.out
