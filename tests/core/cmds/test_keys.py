...
from hydrangea.cmds.hydrangea import cli
from hydrangea.cmds.keys import delete_all_cmd, generate_and_print_cmd, show_cmd, sign_cmd, verify_cmd
from hydrangea.util.config import load_config
from hydrangea.util.file_keyring import FileKeyring
from hydrangea.util.keychain import KeyData, DEFAULT_USER, DEFAULT_SERVICE, Keychain, generate_mnemonic
from hydrangea.util.keyring_wrapper import DEFAULT_KEYS_ROOT_PATH, KeyringWrapper, LegacyKeyring
...
def empty_keyring():
    with TempKeyring(user="user-hydrangea-1.8", service="hydrangea-user-hydrangea-1.8") as keychain:
        yield keychain
...
class TestKeysCommands:
    ...
    def test_show_labels(self, empty_keyring, tmp_path):
        ...
        result = runner.invoke(cli, [*base_params, *cmd_params])
        assert result.output == "No keys are present in the keychain. Generate them with 'hydrangea keys generate'\n"
    ...
    def test_derive_search_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `hydrangea keys derive search` command, searching for a testnet wallet address
        """
        ...
        assert (
            result.output.find(
                (
                    "Found wallet address: "
                    "txhg1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs2v6lg7 (HD path: m/12381/8444/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `hydrangea keys derive wallet-address` command, generating a couple of testnet wallet addresses.
        """
        ...
        assert (
            result.output.find(
                (
                    "Wallet address 50 (m/12381n/8444n/2n/50n): "
                    "txhg1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfshfylvn"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    "Wallet address 51 (m/12381n/8444n/2n/51n): "
                    "txhg1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8s3utl6c"
                )
            )
            != -1
        )
    ...
    def test_migration_incremental(self, tmp_path, keyring_with_one_key, monkeypatch):
        ...
        from hydrangea.util import keyring_wrapper
