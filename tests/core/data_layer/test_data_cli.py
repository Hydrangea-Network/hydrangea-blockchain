...
from tests.core.data_layer.util import HydrangeaRoot

pytestmark = pytest.mark.data_layer


@pytest.mark.asyncio
async def test_help(hydrangea_root: HydrangeaRoot) -> None:
    """Just a trivial test to make sure the subprocessing is at least working and the
    data executable does run.
    """
    completed_process = hydrangea_root.run(args=["data", "--help"])
    assert "Show this message and exit" in completed_process.stdout


@pytest.mark.xfail(strict=True)
@pytest.mark.asyncio
def test_round_trip(hydrangea_root: HydrangeaRoot, hydrangea_daemon: None, hydrangea_data: None) -> None:
    """Create a table, insert a row, get the row by its hash."""

    with hydrangea_root.print_log_after():
        create = hydrangea_root.run(args=["data", "create_data_store"])
