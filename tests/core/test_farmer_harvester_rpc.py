...
from hydrangea.plot_sync.receiver import Receiver
from hydrangea.plotting.util import add_plot_directory
from hydrangea.protocols import farmer_protocol
from hydrangea.protocols.harvester_protocol import Plot
from hydrangea.rpc.farmer_rpc_api import (
    FilterItem,
...
from hydrangea.rpc.farmer_rpc_client import FarmerRpcClient
from hydrangea.rpc.harvester_rpc_client import HarvesterRpcClient
from hydrangea.simulator.block_tools import get_plot_dir
from hydrangea.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
from hydrangea.util.misc import get_list_or_len
from hydrangea.wallet.derive_keys import master_sk_to_wallet_sk, master_sk_to_wallet_sk_unhardened
...
async def test_farmer_signage_point_endpoints(harvester_farmer_environment):
    ...
    sp = farmer_protocol.NewSignagePoint(
        std_hash(b"1"), std_hash(b"2"), std_hash(b"3"), uint64(1), uint64(1000000), uint8(2), bytes32(bytes.fromhex("f422ebc8f2be6c3097dd3ea6caaa519e333684f6fb5a65c9cb66e330d5ba3064")),
    )
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "xhg"), encode_puzzle_hash(new_ph_2, "xhg"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "xhg"), encode_puzzle_hash(observer_pool, "xhg")
    )
    ...
    assert config["farmer"]["xhg_target_address"] == encode_puzzle_hash(observer_farmer, "xhg")
    assert config["pool"]["xhg_target_address"] == encode_puzzle_hash(observer_pool, "xhg")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "xhg")
