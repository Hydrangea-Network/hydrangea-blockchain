...
# Builds CLI only rpm
CLI_RPM_BASE="hydrangea-blockchain-cli-$HYDRANGEA_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/hydrangea"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/hydrangea/"
ln -s ../../opt/hydrangea/hydrangea "dist/$CLI_RPM_BASE/usr/bin/hydrangea"
...
fpm -s dir -t rpm \
  ...
  --name hydrangea-blockchain-cli \
  --license Apache-2.0 \
  --version "$HYDRANGEA_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Hydrangea is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
cd ../hydrangea-blockchain-gui/packages/gui || exit 1
...
PRODUCT_NAME="hydrangea"
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="Hydrangea Blockchain" \
  --config.rpm.packageName="hydrangea-blockchain"
...
GUI_RPM_NAME="hydrangea-blockchain-${HYDRANGEA_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${HYDRANGEA_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
